...
exports.coder = coder;
exports.executeRemotionComposition = executeRemotionComposition;
...
function createCompositionComponent(timeline) {
    return function DynamicComposition(props) {
        ...
        return (react_1.default.createElement("div", { style: {
            ...
            timeline.tracks.map((track, trackIndex) => (react_1.default.createElement("div", { key: track.id, style: { position: 'absolute', width: '100%', height: '100%' } }, track.layers.map((layer, layerIndex) => (react_1.default.createElement(LayerComponent, { key: `${layer.id}-${layerIndex}`, layer: layer, imageUrl: imageUrl, timeline: timeline })))))),
            timeline.globalEffects.map((effect) => (react_1.default.createElement(EffectComponent, { key: effect.id, effect: effect, timeline: timeline })))));
    };
}
function LayerComponent({ layer, imageUrl, timeline }) {
    ...
    if (layer.type === 'image' && layer.assetId) {
        return (react_1.default.createElement("div", { style: {
            ...
            react_1.default.createElement("img", { src: imageUrl, alt: layer.name, style: {
                    width: '100%',
                    height: '100%',
                    objectFit: 'cover',
...
function EffectComponent({ effect, timeline }) {
    const currentTime = 0; // This would be managed by Remotion's useCurrentFrame()
...
function calculateAnimatedStyle(layer, currentTime) {
    const style = {};
    // Find active keyframes
    const activeKeyframes = layer.keyframes.filter(kf => kf.time <= currentTime && (
...
function keyframeToStyle(keyframe) {
    const style = {};
...
function calculateEffectStyle(effect, currentTime) {
    const style = {};
    ...
    switch (effect.type) {
        ...
        case 'colorGrade':
            ...
            if (effect.parameters.temperature) {
                const temperature = effect.parameters.temperature;
...
async function executeRemotionComposition(composition, inputProps) {
