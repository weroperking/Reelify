...
function director(schema, prompt, imagePath) {
    // Create basic timeline structure
    const timeline = (0, schema_1.createBasicTimeline)(5, 1920, 1080, 30);
    ...
    const validation = (0, schema_1.validateMotionIR)(timeline);
...
function extractDurationFromPrompt(prompt) {
...
function determineCameraMovement(direction, schema) {
    const camera = {
        type: '2D',
...
function createMainImageLayer(assetId, duration, direction, schema) {
    const layer = {
        id: 'main-image-layer',
        name: 'Main Image',
        type: 'image',
...
function createSecondaryLayer(elements, duration) {
    const layer = {
...
function createGlobalEffects(direction, duration) {
    const effects = [];
